function BER_MC=MCQPSKrun(N,Eb,No,ChanAtt,...
	TimingBias,TimingJitter,PhaseBias,PhaseJitter,D,ISI)
...
ChanBW = 2*SymRate;					    % bandwidth of channel (Hz)
...
%
SourceBitsI = randi([0 1], 1, SymToSend);
SourceBitsQ = randi([0 1], 1, SymToSend);
%
...
%
[b,a] = butter(5,ChanBW/(fs/2));
if(ISI == 0)
        b=[1]; a=[1];									% filter bypassed 
end
[~,FilterState]=filter(b,a,0);
%
